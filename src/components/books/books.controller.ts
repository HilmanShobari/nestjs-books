...
export class BooksController {
  ...
  async create(@Body() createBookDto: CreateBookDto, @Res() res: Response) {
    try {
      ...
      return res.status(HttpStatus.CREATED).json({ message: "Book created successfully", book: newBook });
    } catch (error) {
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ error: "An error occurred while creating the book" });
    }
  }
